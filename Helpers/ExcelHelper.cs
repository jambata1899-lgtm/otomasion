...
namespace FinalDotnetCoreBuild.Helpers
{
    ...
    public static class ExcelHelper
    {
        ...
        public static List<Letter> Load()
        {
            ...
            while (true)
            {
                var stableKeyCell = ws.Cell(row,1);
                if (stableKeyCell.IsEmpty()) break;

                try
                {
                    var l = new Letter();
                    l.StableKey = ws.Cell(row,1).GetString();
                    ...
                    l.Status = Enum.TryParse<LetterStatus>(ws.Cell(row,9).GetString(), out var st) ? st : LetterStatus.در_حال_پیگیری;
